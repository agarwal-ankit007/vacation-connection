...
<?php 
            ...
            $sql1 ="select c.type , c.source , c.destination , bc.date , bc.bid 
from cars c , books_car bc
where c.car_id = bc.car_id and bc.cus_id = '$cus_id';";
...
$count=0;
     ...
     echo '<div class="main_out"><div class="top">Car Bookings</div>';
    
for($i=0;$i<$rnum;$i++)
{

    
echo '<div class="out"><div class="row"><div class="icon"><i class="fa fa-car" aria-hidden="true"></i></div><div class="short_detail">'?><?php echo $source[$i]." "." to ".$destination[$i]." <i class=\"fa fa-long-arrow-right\" aria-hidden=\"true\"></i> ".$type[$i]." <i class=\"fa fa-long-arrow-right\" aria-hidden=\"true\"></i> ".$date[$i];  echo "</div>"; ?><?php 


    echo '<div class="detail"><a href="car_main_details.php?bid='?>
...
if($date[$i]>$date_check)
{
    ...
    if(isset($_POST["cancel$i"]))
    {
        ...
        $query = "CALL delete_carbooking($bid[$i]);";
        mysqli_query($conn,$query);
        
       
        
        echo '<script>
        window.location = "car_details.php";
