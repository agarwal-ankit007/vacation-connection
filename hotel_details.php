...
<?php 
            ...
            $sql1 ="select h.name , h.location , bh.check_in , bh.bid 
from hotel h , books_hotel bh
where h.hid = bh.hid and bh.cus_id = '$cus_id';";
...
$count=0;
    ...
    echo '<div class="main_out"><div class="top">Hotel Bookings</div>';
    
for($i=0;$i<$rnum;$i++)
{

    
echo '<div class="out"><div class="row"><div class="icon"><i class="fa fa-hotel" aria-hidden="true"></i></div><div class="short_detail">'?><?php echo $location[$i]." <i class=\"fa fa-long-arrow-right\" aria-hidden=\"true\"></i> "  .$hname[$i]." <i class=\"fa fa-long-arrow-right\" aria-hidden=\"true\"></i> ".$check_in[$i];echo "</div>"; ?><?php 


    echo '<div class="detail"><a href="hotel_main_details.php?bid='?>
<?php echo $bid[$i];?><?php echo '">';echo "details"; echo "</a></div>";
if($check_in[$i]>$date_check)
{
    ...
    if(isset($_POST["cancel$i"]))
    {
        ...
        $query = "CALL delete_hbooking($bid[$i]);";
        mysqli_query($conn,$query);
        
       
        
        echo '<script>
        window.location = "hotel_details.php";
