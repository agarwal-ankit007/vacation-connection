...
<?php 
            ...
            $sql1 ="select a.name , a.location , ba.date , ba.bid 
from activities a , books_activities ba
where a.act_id = ba.act_id and ba.cus_id = '$cus_id';";
...
$count=0;
    ...
    echo '<div class="main_out"><div class="top">Activity Bookings</div>';
    
for($i=0;$i<$rnum;$i++)
{

    
echo '<div class="out"><div class="row"><div class="icon"><i class="fa fa-tasks" aria-hidden="true"></i></div><div class="short_detail">'?><?php echo $location[$i]." <i class=\"fa fa-long-arrow-right\" aria-hidden=\"true\"></i> "."  ".$aname[$i]." <i class=\"fa fa-long-arrow-right\" aria-hidden=\"true\"></i> ".$date[$i]; echo "</div>"; ?><?php 


    echo '<div class="detail"><a href="activity_main_details.php?bid='?>
<?php echo $bid[$i];?><?php echo '">';echo "details"; echo "</a></div>";
if($date[$i]>$date_check)
{
    ...
    if(isset($_POST["cancel$i"]))
    {
        ...
        $query = "CALL delete_actbooking($bid[$i]);";
        mysqli_query($conn,$query);
        
       
        
        echo '<script>
        window.location = "activity_details.php";
